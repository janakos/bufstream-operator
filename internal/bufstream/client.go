...
func NewClient(ctx context.Context, bootstrapServers string) (*Client, error) {
	...
	// Custom dialer that redirects cluster DNS to localhost for local development
	customDialer := &net.Dialer{Timeout: 5 * time.Second}
	dialFunc := func(ctx context.Context, network, address string) (net.Conn, error) {
		// If the address contains cluster DNS (case-insensitive), replace with localhost
		lowerAddress := strings.ToLower(address)
		if strings.Contains(lowerAddress, ".svc.cluster.local:") {
